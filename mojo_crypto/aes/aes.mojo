...
struct Aes[KeySize: Int](BlockCipher, GpuBlockCipher, ImplicitlyDestructible):
    ...
    var _gpu: Optional[AesGpuSetup] = None
    ...
    def encrypt[
        ...
        for i in range(Size // BLOCK_SIZE):
            ...
            var enc = cpu_cipher[Nr=Self.Nr](block, self.w)
            for j in range(BLOCK_SIZE):
                result[i * BLOCK_SIZE + j] = enc[j]
        return result
    ...
    def decrypt[
        ...
        for i in range(Size // BLOCK_SIZE):
            ...
            for j in range(BLOCK_SIZE):
                block[j] = data[i * BLOCK_SIZE + j]
            var dec = cpu_decipher[Nr=Self.Nr](block, self.w)
            for j in range(BLOCK_SIZE):
                result[i * BLOCK_SIZE + j] = dec[j]
        return result
