...
def _aese(
    ...
    return llvm_intrinsic["llvm.aarch64.crypto.aese", SIMD[DType.uint8, BLOCK_SIZE]](
        state, key
    )
...
@always_inline
def _aesmc(state: SIMD[DType.uint8, BLOCK_SIZE]) -> SIMD[DType.uint8, BLOCK_SIZE]:
    return llvm_intrinsic["llvm.aarch64.crypto.aesmc", SIMD[DType.uint8, BLOCK_SIZE]](
        state
    )
...
def _aesd(
    ...
    return llvm_intrinsic["llvm.aarch64.crypto.aesd", SIMD[DType.uint8, BLOCK_SIZE]](
        state, key
    )
...
def _inv_mix(v: SIMD[DType.uint8, BLOCK_SIZE]) -> SIMD[DType.uint8, BLOCK_SIZE]:
    return llvm_intrinsic["llvm.aarch64.crypto.aesimc", SIMD[DType.uint8, BLOCK_SIZE]](
        v
    )
