...
def load_2(dir: str, test_type: TestType) -> list[dict]:
    # Unlike load(), this does no AFT/MCT-specific field extraction or
    # renaming (pt_hex/ct_hex/is_encrypt/...): each record is just the raw
    # group, test-case, and expected-result dicts as read from JSON, keyed
    # by tcId. Interpretation of these fields happens on the Mojo side.
    root = Path(dir)
