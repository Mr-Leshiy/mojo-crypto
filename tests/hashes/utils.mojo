...
def run_sha224_checks[
    ...
    comptime if target_triple_contains_any(["aarch64", "arm64"]) and has_target_feature["sha2"]():
        check[Sha224Aarch64](vectors)

    comptime if target_triple_contains_any(["x86_64"]) and has_target_feature["sha"]():
...
def run_sha384_checks[
    ...
    comptime if target_triple_contains_any(["aarch64", "arm64"]) and has_target_feature["sha3"]():
        check[Sha384Aarch64](vectors)

    comptime if target_triple_contains_any(["x86_64"]) and has_target_feature["sha512"]():
